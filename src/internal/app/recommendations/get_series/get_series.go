...
func GetSeries(data models.GetSeriesRequest) models.GetSeriesResponse {
	var response models.GetSeriesResponse
	...
	config := rec_db_config.GetConfigAPI("Series-API")
	collection := connect_to_mongodb.CreateConnection(config)

	var film_results []models.Series
